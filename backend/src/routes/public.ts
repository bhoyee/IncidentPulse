...
const publicRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // JSON payload for the embeddable widget
    ...
    return reply.send({
      ...
      data: {
        ...
        branding: {
          ...
          textColor: settings?.statusTextColor ?? "#e2e8f0",
          backgroundColor: settings?.statusBackgroundColor ?? "#0f172a",
        },
      },
    });
  });
  ...
  styles.textContent='\\
    ...
    .ip-service{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;padding:10px 12px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.02);}\\
    ...
    .ip-maint-card{border:1px solid rgba(255,255,255,0.08);border-radius:12px;overflow:hidden;background:rgba(255,255,255,0.03);}\\
  ...
  fetch('${apiBase}/public/status/embed?orgSlug='+encodeURIComponent(org))
    ...
    .then(payload=>{
      ...
      // days since last incident: 0 if active, else from historical incidents if available
      ...
      const svcHtml=services.map((s,svcIdx)=>{
        ...
        return '<div class=\"ip-service\" data-svc-idx=\"'+svcIdx+'\" style=\"color:'+fg+';border-color:'+fg+'22;background:#fff;\">'
          + '<div class=\"ip-service-content\"><div style=\"font-weight:700;\">'+(s.name||'Service')+'</div><div class=\"ip-uptime-bars\">'+bars+'</div></div>'
      ...
      const maintHtml=maint.length===0
        ...
        : maint.map(m=>{
            ...
            const maintHeaderColor = '#0ea5e9'; // sky blue for maintenance header
      ...
      container.innerHTML='<div class=\"ip-wrap\" style=\"color:'+fg+';\">'
        ...
        + '</div>'
        + '<div class=\"ip-stats\">'
        + '<div class=\"ip-stat-card\" style=\"border-color:'+fg+'22;background:#fff;\"><p class=\"ip-stat-value\" style=\"color:'+fg+';\">'+incidents.length+'</p><p class=\"ip-stat-label\">Active incidents</p></div>'
        + '<div class=\"ip-stat-card\" style=\"border-color:'+fg+'22;background:#fff;\"><p class=\"ip-stat-value\" style=\"color:'+fg+';\">'+(maint.length||0)+'</p><p class=\"ip-stat-label\">Active maintenance</p></div>'
        + '<div class=\"ip-stat-card\" style=\"border-color:'+fg+'22;background:#fff;\"><p class=\"ip-stat-value\" style=\"color:'+fg+';\">'+daysSince+'</p><p class=\"ip-stat-label\">Days since last incident</p></div>'
        + '</div>'
        + '<div class=\"ip-section\" style=\"background:#fff;\"><h3 style=\"color:'+fg+';\">Active incidents</h3>'+incHtml+'</div>'
        + '<div class=\"ip-section\" style=\"background:#fff;\"><h3 style=\"color:'+fg+';\">Services & uptime</h3><div class=\"ip-services\">'+svcHtml+'</div></div>'
        + '<div class=\"ip-section\" style=\"background:#fff;\"><h3 style=\"color:'+fg+';\">Scheduled maintenance</h3><div class=\"ip-maint-list\">'+maintHtml+'</div></div>'
